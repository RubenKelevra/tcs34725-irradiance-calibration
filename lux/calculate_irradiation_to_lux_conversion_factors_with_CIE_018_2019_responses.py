...
# Normalize the scaled spectral responses to have a maximum of 1.0
red_response_norm = red_response_scaled / np.max(np.abs(red_response_scaled))
green_response_norm = green_response_scaled / np.max(np.abs(green_response_scaled))
blue_response_norm = blue_response_scaled / np.max(np.abs(blue_response_scaled))
...
# For standard illuminant A (tungsten light), the conversion is approximately 0.0079 W/m² per lux
# For daylight (D65), it's approximately 0.0075 W/m² per lux
# Let's use an average value of 0.0077 W/m² per lux
# Since our irradiance is in μW/cm², we need to convert units
# 1 W/m² = 100 μW/cm²

irradiance_per_lux = 0.005  # W/m² per lux

# Convert W/m² to μW/cm²
irradiance_per_lux_uW_cm2 = irradiance_per_lux * 100  # μW/cm² per lux

# Compute the conversion factors from irradiance (μW/cm²) to illuminance (lux)
...
print(f"Red channel:   K_red = {K_red:.4f} lux per μW/cm²")
print(f"Green channel: K_green = {K_green:.4f} lux per μW/cm²")
print(f"Blue channel:  K_blue = {K_blue:.4f} lux per μW/cm²")
